...
try {
    $db->prepare('UPDATE device_config SET ota_update_on_sync = ? WHERE device_id = ?')
       ->execute([$enable ? 1 : 0, $device_id]);

    echo json_encode([
        'ok'                => true,
        'device_id'         => $device_id,
        'ota_update_on_sync' => $enable,
    ]);

} catch (PDOException $e) {
    error_log('[SMCR API] DB error in set_ota_on_sync: ' . $e->getMessage());
