...
try {
    ...
    $response = [
        ...
        'wifi_offline_restart_min'  => (int)$cfg['wifi_offline_restart_min'],
        'ap_ssid'               => $cfg['ap_ssid'],
        'ap_pass'               => $cfg['ap_pass'],
    ...
    if (!empty($cfg['pending_cloud_migration'])) {
        ...
        $response['cloud_port']      = (int)($cfg['cloud_port']      ?? 80);
    ...
    if (!empty($cfg['fetch_html_on_sync'])) {
        $db->prepare('UPDATE device_config SET fetch_html_on_sync = 0 WHERE device_id = ?')
           ->execute([$device_id]);
    }
